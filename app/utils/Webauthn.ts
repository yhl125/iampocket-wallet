...
import { WebAuthnAuthenticationVerificationParams } from '@lit-protocol/types';
...
export async function pollRequestUntilTerminalState(requestId: string) {
...
// sessionSigs currently not working with PKPEthersWallet
// when PKPEthersWallet supports sessionSigs, migrate from authSig to sessionSigs
export async function getSessionSigsForWebAuthn(
  pkpPublicKey: string,
  authData: WebAuthnAuthenticationVerificationParams,
  litResource: LitPKPResource
) {
...
export async function getAuthSigForWebAuthn(
  authData: WebAuthnAuthenticationVerificationParams
) {
  const litNodeClient = new LitNodeClient({
    litNetwork: 'serrano',
...
export async function fetchPKPs(
  authData: WebAuthnAuthenticationVerificationParams
) {
...
function getDomainFromOrigin(origin: string) {
